...
Unlike a virtual machine, a container **does not virtualize hardware and does not ship a kernel**: all containers share the host's Linux kernel. A container is just a regular process, isolated by kernel mechanisms. The result: startup in milliseconds, a few MB of overhead, and far higher density than VMs.
...
The lifecycle: a `Dockerfile` describes the build → `docker build` produces an **image** (immutable, versioned by tag and digest) → `docker run` instantiates a **container** (ephemeral) → the image is distributed through a **registry** (Docker Hub, GHCR, private registry).

Architecture: the `docker` CLI talks to a daemon (`dockerd`) over a REST API; the daemon delegates execution to `containerd` then `runc` (the OCI standard). That's why Kubernetes can use containerd without Docker.
...
- **Docker publishes ports bypassing the UFW firewall**: `-p 0.0.0.0:5432:5432` exposes your database to the Internet even if UFW blocks it (iptables rules are inserted first). Bind to `127.0.0.1:5432:5432` for local-only services.
