...
- **Strings** — valeur binaire quelconque ; `INCR` atomique → compteurs, rate limiting.
- **Hashes** — champ → valeur (`HSET user:42 name "Ada" role "dev"`) : l'objet léger.
- **Lists** — deque ; `LPUSH` + `BRPOP` (pop bloquant) = file de tâches basique.
- **Sets** — unicité et opérations ensemblistes (`SINTER`) : tags, visiteurs uniques.
- **Sorted sets** — membres triés par score : leaderboards (`ZADD`/`ZRANGE`), rate limiting en sliding window, files à priorité.
- **Streams** — log append-only avec **consumer groups** et acquittements (`XADD`/`XREADGROUP`/`XACK`) : ce qui se rapproche le plus d'une vraie file de messages.
...
- **Cache sans TTL** : la mémoire se remplit inexorablement ; avec `noeviction` (défaut) les écritures se mettent à échouer, avec `allkeys-lru` des données qu'on croyait durables disparaissent en silence. Un cache sans TTL est une fuite mémoire polie.
