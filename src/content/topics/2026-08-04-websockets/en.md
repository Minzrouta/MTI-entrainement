...
- **Short polling**: the client asks every X seconds. Simple, but average latency of X/2 and piles of empty requests.
- **Long polling**: the server holds the request open until it has data (or times out), and the client immediately calls back. Near-zero latency, but one HTTP connection consumed per client and one reconnection per message.
- **SSE (Server-Sent Events)**: a `text/event-stream` flow over a plain HTTP request; the browser's `EventSource` API handles automatic reconnection and resumption via `Last-Event-ID`. Unidirectional server → client, text only. Underrated: perfect for notifications, dashboards, streaming LLM responses. The historical 6-connections-per-domain limit of HTTP/1.1 is lifted by HTTP/2 multiplexing.
- **WebSocket**: bidirectional, binary or text, minimal overhead. The right choice when the *client* also sends frequently: chat, games, collaborative editing.
...
- **Neglecting `Origin`**: WebSockets are not subject to CORS. A server that authenticates via cookies without checking the origin is vulnerable to cross-site WebSocket hijacking.
