...
- **Short polling** : le client interroge toutes les X secondes. Simple, mais latence moyenne de X/2 et requêtes vides en masse.
- **Long polling** : le serveur garde la requête ouverte jusqu'à avoir une donnée (ou timeout), le client rappelle aussitôt. Latence quasi nulle, mais une connexion HTTP consommée par client et une reconnexion par message.
- **SSE (Server-Sent Events)** : flux `text/event-stream` sur une simple requête HTTP ; l'API `EventSource` du navigateur gère la reconnexion automatique et la reprise via `Last-Event-ID`. Unidirectionnel serveur → client, texte seulement. Sous-estimé : parfait pour notifications, dashboards, streaming de réponses LLM. Limite historique des 6 connexions par domaine en HTTP/1.1, levée par le multiplexage HTTP/2.
- **WebSocket** : bidirectionnel, binaire ou texte, overhead minimal. Le bon choix quand le *client* émet aussi fréquemment : chat, jeux, édition collaborative.
...
- **Négliger `Origin`** : les WebSockets ne sont pas soumis au CORS. Un serveur qui authentifie par cookie sans vérifier l'origine est vulnérable au cross-site WebSocket hijacking.
